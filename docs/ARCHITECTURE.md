...
```
Workout Editor ──export──▶ Garmin Connect workout JSON (file)
      ▲                            │
      │                    "Share your Garmin Connect workout"
   import                  Chrome extension (user's Connect session)
      │                            ▼
...
- Deferred until requested: `.FIT` export for USB copy (Garmin's FIT JS SDK has a
...
```
packages/
  core/        # @workout-editor/core — no React, no DOM
    src/model/     # canonical Workout/Step/Target types (Zod schemas = validation + TS types)
    src/sports/    # sport plugin definitions; strength first
    src/connect/   # canonical model ⇄ Garmin Connect workout JSON (two-way)
    src/exercises/ # exercise taxonomy (from Connect fixtures; FIT SDK Profile as dev-time source)
...
2. **Sport as plugin.** A sport contributes: the step/target types it allows, editor field
   components, and format conversion. Strength is simply the first plugin; the contract will
...
| UI          | React 19 + Vite 8                 |                                                                                                     |
| Lint        | oxlint                            | what the Vite template ships now (2026); faster than ESLint, zero-config                            |
